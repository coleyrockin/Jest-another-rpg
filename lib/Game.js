...
class Game {
  ...
  showStateCard() {
    ...
    if (this.pacing === 'quick') {
      ...
      this.showStatus(
        `HP ${stats.health}/${this.player.maxHealth} | ${region.name} | Enemy ${enemyName} ${enemyHealth} | Lvl ${stats.level} XP ${stats.xp}/${stats.xpToNext}`
      );
    ...
    this.showStatus(`Region: ${summarizeRegion(this.worldState.currentRegion)}`);
    this.showStatus(
      `HP: ${stats.health}/${this.player.maxHealth} | STR: ${stats.strength} | AGI: ${stats.agility} | Gold: ${stats.gold}g`
    );
    this.showStatus(
  ...
  async adventureHub() {
    ...
    while (choosing) {
      const { action } = await this.inquirer.prompt(prompts.adventureAction());
      ...
      if (action === 'travel') {
        ...
        this.showStatus(`Traveled to ${summarizeRegion(regionId)}`);
