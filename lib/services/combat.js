...
function calculateDodgeChance(attacker, defender) {
  ...
  const dodgeProfile = defender.className
    ? getClassProfile(defender.className).dodgeBonus
    : 0;
  const opportunistBonus =
    defender.aiProfile === 'opportunist' && defender.agility > attacker.agility
      ? 20
      : 0;
...
function resolveCombatTurn({
  ...
  if (actorType === 'player' && action === 'item') {
    ...
    return {
      ...
      crit: false,
      damage: 0,
      statusApplied: result ? [result.result.effect] : [],
      stateUpdated,
      message: result
        ? `Used ${result.item.name}.`
        : 'No item available for that slot.',
      action: 'item'
    };
  }
...
function selectEnemyAction(enemy, player, rng) {
  const profile = {
    aggressive: () => 'attack',
    tactical: () =>
      enemy.health / enemy.maxHealth < 0.35 && rng.chance(30) ? 'focus' : 'attack',
    opportunist: () => 'attack'
  };
  const selected =
