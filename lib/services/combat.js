...
function applyMitigation(target, damage) {
  const equipmentDefense =
    typeof target.getEquipmentDefense === 'function' ? target.getEquipmentDefense() : 0;
  const defendedDamage = Math.max(1, damage - equipmentDefense);

  if (target.hasStatus('defending')) {
    return Math.max(1, defendedDamage - 2);
  }
